...
def make_problem(k: int = 2):
    ...
    # Add weak noise to smear path weights 
    noise = NoiseModel(processes=[
        {"name": "z", "sites": [i], "strength": 0.5} for i in range(op.length)
    ])
    params = AnalogSimParams(dt=0.1, solver="MCWF", show_progress=False)
    return op, params, [0.1] * k, noise


def dense_ref_total(k: int, op, params, timesteps, noise) -> np.ndarray:
    ...
    U_ref, _ = run_mc_upsilon(
        ...
        noise_model=noise, dual_transform="T", replace=False,
    )
    return U_ref
...
def main() -> None:
    ...
    k = args.k
    n_seeds = args.seeds
    op, params, timesteps, noise = make_problem(k)

    print(f"Building dense reference (k={k}) TOTAL T…")
    t0 = time.perf_counter()
    U_ref = dense_ref_total(k, op, params, timesteps, noise)
    ...
    sample_ns = np.array([16, 32, 64, 128, 256, 512])
    ...
    def sis_fn(N: int, seed: int):
        return run_sis_upsilon(
            op, params, timesteps=timesteps,
            num_particles=N, noise_model=noise, seed=seed,
    ...
    ax.set_title(f"Process-tensor convergence  |  k={k}  |  noisy evolution")
    ax.legend()
    ...
    ax.set_title(f"QMI convergence  |  k={k}  |  noisy evolution")
    ax.legend()
    ax.grid(True, which="both", alpha=0.35)
    fig.tight_layout()
    ...
    log_N = np.log(sample_ns.astype(float))
    for name, data in [("MC Fro", mc["fro_mu"]), ("SIS Fro", sis["fro_mu"]),
                       ("MC |QMI|", mc["qmi_mu"]), ("SIS |QMI|", sis["qmi_mu"])]:
        slope = float(np.polyfit(log_N, np.log(data), 1)[0])
        print(f"  log-log slope {name:12s}: {slope:.3f}  (ideal = -0.500)")
