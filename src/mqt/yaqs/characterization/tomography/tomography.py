...
def run_mc_upsilon_mpo(
    ...
    """Run Monte Carlo Process Tomography returning an MPO.

    Accumulates the comb Choi operator as a Matrix Product Operator (MPO)
    to avoid the O(4^k) dense memory cost. The running MPO is periodically compressed
    to bound the bond dimension.
    ...
    Args:
        ...
        noise_model: Noise model to apply.
        seed: Random seed for sampling.
        dual_transform: Transformation to apply to dual matrices.
        replace: Whether to sample sequences with replacement.
        ensemble: Basis sequence ensemble to sample from ("discrete" or "continuous").
        sampling: Sampling mode ("uniform" or "candidate_local").
        num_candidates: Number of candidates for local importance sampling.
        compress_every: Frequency of SVD compression (every N sequences).
        tol: SVD truncation threshold during compression.
        max_bond_dim: Hard cap on MPO bond dimension during compression.
        n_sweeps: Number of SVD sweeps per compression step.

    Returns:
        tuple containing:
        - running_mpo: Reconstructed process tensor as an MPO.
        - meta: Dictionary containing simulation metadata.
    """
    if timesteps is None:
        timesteps = [sim_params.elapsed_time]
    ...
    results_iterator = run_backend_parallel(
        ...
        max_workers=max_workers,
        show_progress=sim_params.show_progress,
        desc=f"Simulating {num_sequences} MC Sequences (MPO)",
    )

    # 5. Build MPO
    # Collect discrete runs to average them per-sequence before building MPO
    avg_rho_weighted = np.zeros((num_sequences, 2, 2), dtype=np.complex128)
    uncompressed_mpos: list[MPO] = []
    running_mpo: MPO | None = None
    
    def _flush_mpos():
        nonlocal running_mpo, uncompressed_mpos
        if not uncompressed_mpos:
            return
        
        chunk_sum = MPO.mpo_sum(uncompressed_mpos)
        uncompressed_mpos.clear()
        
        if running_mpo is None:
            running_mpo = chunk_sum
        else:
            running_mpo = running_mpo + chunk_sum
            
        running_mpo.compress(tol=tol, max_bond_dim=max_bond_dim, n_sweeps=n_sweeps)

    # Fast dual operator mappings
    def _get_dual(psi_m: NDArray[np.complex128], psi_p: NDArray[np.complex128]) -> NDArray[np.complex128]:
        P = np.outer(psi_m, psi_m.conj())
        Q = np.outer(psi_p, psi_p.conj())
        D_Q = 2.0 * (3.0 * Q - np.eye(2, dtype=np.complex128))
    ...
    for _job_idx, (s_idx, _traj_idx, _alpha, rho_final, sequence_weight, seq_states) in results_iterator:
        if ensemble == "continuous":
            assert seq_states is not None
            ops = [_apply_dual_transform(_get_dual(m, p), dual_transform) for m, p in seq_states]
            weight = sequence_weight * (inv_q / (num_sequences * num_trajectories))
            term = rank1_upsilon_mpo_term(rho_final, ops, weight=weight)
            uncompressed_mpos.append(term)
            
            if len(uncompressed_mpos) >= compress_every:
                _flush_mpos()
        else:
            # Aggregate discrete responses per sequence before building the rank-1 term
            avg_rho_weighted[s_idx] += rho_final * sequence_weight

    if ensemble == "discrete":
        avg_rho_weighted /= num_trajectories
        for s_idx, alpha_seq in enumerate(sampled_sequences):
            ops = [_apply_dual_transform(duals[a], dual_transform) for a in alpha_seq]
            weight = inv_q / num_sequences
            term = rank1_upsilon_mpo_term(avg_rho_weighted[s_idx], ops, weight=weight)
            uncompressed_mpos.append(term)
    ...
    # If NO samples were valid (extreme case), return zero MPO
    if running_mpo is None:
        running_mpo = rank1_upsilon_mpo_term(
            np.zeros((2, 2)), 
            [np.zeros((4,4)) for _ in range(k)], 
            0.0
        )
    ...
    meta = {
        ...
        "seed": seed,
        "ensemble": ensemble,
        "bond_dim_final": [t.shape[3] for t in running_mpo.tensors[:-1]],
        "max_bond_final": max([t.shape[3] for t in running_mpo.tensors[:-1]]) if len(running_mpo.tensors) > 1 else 1,
        "compression_tol": tol,
        "compression_max_bond_dim": max_bond_dim,
        "compression_n_sweeps": n_sweeps,
    }

    return running_mpo, meta
...
def run_sis_upsilon_mpo(
    ...
    **Rejuvenation** (``rejuvenate=True``): after resampling, propose an
    ...
    if rejuvenate:
        raise NotImplementedError("rejuvenate=True not yet supported in run_sis_upsilon_mpo; update past_local_ops consistently first.")
    if proposal not in ("uniform", "local", "mixture"):
        raise ValueError(f"proposal must be 'uniform', 'local', or 'mixture', got '{proposal}'.")
    ...
    for step_i, duration in enumerate(timesteps):
        ...
        if w_sum < 1e-30:
            weights[:] = 1.0
            states = np.tile(psi_zero, (num_particles, 1))
            past_local_ops = [[] for _ in range(num_particles)]  # no aliasing
            alpha_seqs = [[] for _ in range(num_particles)]
            checkpoints.clear()
            ess_history.append(float(num_particles))
            weight_cv.append(0.0)
    ...
    for i in range(num_particles):
        rho_mat = np.reshape(states[i], (2, -1))
        rho_i = rho_mat @ rho_mat.conj().T
        weight = weights[i] / float(num_particles)
        
        term = rank1_upsilon_mpo_term(rho_i, past_local_ops[i], weight=weight)
        uncompressed_mpos.append(term)
        if len(uncompressed_mpos) >= compress_every:
            _flush_mpos()
    ...
    # If returned None (0 particles, impossible due to checks but type-safety here)
    if running_mpo is None:
        running_mpo = rank1_upsilon_mpo_term(
            np.zeros((2, 2)), 
            [np.zeros((4,4)) for _ in range(k)], 
    ...
    meta = {
        ...
        "trace_before_resampling": trace_before_resampling,
        "total_weight": float(weights.sum()) / num_particles,
        "bond_dim_final": [t.shape[3] for t in upsilon.tensors[:-1]],
        "seed": seed,
        "max_bond_final": max([t.shape[3] for t in upsilon.tensors[:-1]]) if len(upsilon.tensors) > 1 else 1,
        "compression_tol": tol,
        "compression_max_bond_dim": max_bond_dim,
        "compression_n_sweeps": n_sweeps,
    }
