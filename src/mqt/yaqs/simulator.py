...
def _unitary_ensemble_worker(job_idx: int) -> tuple[NDArray[np.float64], NDArray[np.complex128] | None]:
    ...
    Returns:
        tuple[NDArray[np.float64], NDArray[np.complex128] | None]:
            Observable trajectories for one member and optional autocorrelator trajectory for the
            same member.
    """
    ...
    *,
    ...
    Raises:
        ValueError: If noisy simulation is requested with a list of states, if ``solver`` is
            ...
            if ``get_state`` is requested, or if autocorrelator options are inconsistent.
    """
    ...
    if parallel and len(initial_states) > 1:
        max_workers = max(1, available_cpus() - 1)
        for i, (obs_result, autocorr_result) in run_backend_parallel(
        ...
        for i, arg in enumerate(iterator):
            obs_result, autocorr_result = _call_backend(unitary_ensemble_member_worker, arg, n_threads=n_threads)
            for obs_index, observable in enumerate(sim_params.sorted_observables):
                assert observable.trajectories is not None, "Trajectories should have been initialized"
                observable.trajectories[i] = obs_result[obs_index]
            if autocorr_matrix is not None:
