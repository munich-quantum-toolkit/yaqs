...
import numpy as np
from mqt.noisy_qc_sim.krauschannel_simulation import (
    circuit_to_unitary_list,
    create_all_zero_density_matrix,
    evolve_noisy_circuit,
    get_qiskit_z_expectations,
    z_expectations,
)
from qiskit import QuantumCircuit


def run_test(test_name, num_qubits, circuit_builder, expected_z_qiskit) -> None:
...
if __name__ == "__main__":
    ...
    run_test(
        "Custom Circuit (Original Example)",
        n_custom,
    ...
    run_test(
        "3-Qubit Mixed Pauli + Entangling",
        n10,
    ...
    run_test(
        "4-Qubit Linear Chain + Rotations",
        n11,
        lambda circ: (
            circ.h(0),
            circ.cx(0, 1),
    ...
    run_test(
        "3-Qubit W State",
        n12,
        lambda circ: (circ.ry(np.arccos(np.sqrt(2 / 3)), 0), circ.cz(0, 1), circ.cx(1, 2), circ.x(0)),
        expected_z12,
    )
    ...
    run_test(
        "4-Qubit Star Topology + Rotations",
        n13,
        lambda circ: (
            circ.h(0),
            circ.cx(0, 1),
    ...
    run_test(
        "3-Qubit QFT Circuit",
        n15,
        lambda circ: (circ.x(0), circ.h(1), circ.h(0), circ.h(1), circ.h(2), circ.swap(1, 2)),
        expected_z15,
    )
    ...
    run_test("4-Qubit Toffoli Chain", n16, lambda circ: (circ.h(0), circ.h(1), circ.rz(np.pi / 3, 2)), expected_z16)
