...
class MPO:
    ...
    def from_matrix(
        ...
        if not (np.isclose(n_rows, n) and np.isclose(n_cols, n)):
            msg = f"Matrix dimensions are not compatible with uniform (d_in={d_in}, d_out={d_out}) MPO."
            raise ValueError(msg)
        ...
        T_last = np.transpose(rem, (1, 2, 0)).reshape(d_in, d_out, left_rank, 1).astype(dtype, copy=False)
