...
class MPO:
    ...
    def from_pauli_sum(
        ...
        *,
        ...
        bond_state_maps: list[dict[tuple[str, int], int]] = [
    ...
    def from_matrix(
        ...
        """
        Factorize a dense matrix into an MPO with uniform local dimensions.

        Each site has shape (d_in, d_out). The number of sites n is inferred from

            mat.shape = (d_in**n, d_out**n)

        Parameters
        ----------
        mat : np.ndarray
            Matrix of shape (d_in**n, d_out**n).
        d_in : int
            Input physical dimension per site.
        d_out : int
            Output physical dimension per site.
        ...
        # --- infer length ---
        n_rows = np.log(rows) / np.log(d_in)
        n_cols = np.log(cols) / np.log(d_out)

        n = int(round(n_rows))

        if not (np.isclose(n_rows, n) and np.isclose(n_cols, n)):
            raise ValueError(
                "Matrix dimensions are not compatible with uniform "
                f"(d_in={d_in}, d_out={d_out}) MPO."
            )

        M = np.asarray(mat, dtype=dtype, order="F")
        ...
        # identical SVD sweep as before
        for k in range(n - 1):
            rest_rows = d_in ** (n - k - 1)
            rest_cols = d_out ** (n - k - 1)

            rem = rem.reshape(left_rank, d_in, rest_rows, d_out, rest_cols)

            rem_perm = np.transpose(rem, (1, 3, 0, 2, 4))
            X = rem_perm.reshape(d_in * d_out * left_rank, rest_rows * rest_cols)
            ...
            T_k = U.reshape(d_in, d_out, left_rank, r_keep).astype(dtype, copy=False)
            tensors.append(T_k)

            rem = (s[:, None] * Vh).reshape(r_keep, rest_rows, rest_cols)
            left_rank = r_keep

        rem = rem.reshape(left_rank, d_in, d_out)

        T_last = (
            np.transpose(rem, (1, 2, 0))
            .reshape(d_in, d_out, left_rank, 1)
        ...
        mpo.tensors = tensors
        mpo.length = n
        mpo.physical_dimension = (d_in, d_out)
