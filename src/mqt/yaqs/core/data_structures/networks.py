...
class MPO:
    ...
    def from_matrix(
        ...
        Raises:
            ValueError:
                If ``d <= 0``;
                If ``d == 1`` but the matrix is not ``1 × 1``;
        ...
        # --- validate d ---
        if d <= 0:
            raise ValueError(f"Physical dimension d must be > 0, got d={d}.")

        rows, cols = mat.shape

        if rows != cols:
            raise ValueError("Matrix must be square for uniform MPO factorization.")

        # Special case: d == 1
        if d == 1:
            if rows != 1:
                raise ValueError(
                    "For d == 1 the matrix must be 1×1 since 1**n = 1 for any n."
                )
            n = 1
        else:
            # --- infer chain length ---
            ...
            if n < 1:
                raise ValueError(
                    f"Inferred chain length n={n} is invalid; "
                    f"matrix dimension {rows} too small for base d={d}."
                )
