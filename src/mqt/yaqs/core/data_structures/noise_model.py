...
class CompactNoiseModel:
    """Compact multi-site noise specification for parameter optimization."""

    def __init__(self, compact_processes: list[dict[str, Any]] | None = None) -> None:
        """Initialize a compact noise model and expand it to per-site processes.

        Args:
            compact_processes: List of compact process dicts. Each entry must contain
                ``name`` (a :class:`~mqt.yaqs.core.libraries.noise_library.NoiseLibrary`
                entry), ``sites``, and ``strength``. For one-site processes ``sites`` is
                a flat list of site indices; for two-site processes it is a list of
                ``[site_a, site_b]`` pairs.
        ...
        for i, proc in enumerate(self.compact_processes):
            ...
            name = proc["name"]
            if not hasattr(NoiseLibrary, name):
                msg = f"Noise process '{name}' not found in NoiseLibrary"
                raise ValueError(msg)

            sites_spec = proc["sites"]
            if not sites_spec:
                msg = "Each process must list at least one site or site-pair"
                raise ValueError(msg)

            if isinstance(sites_spec[0], int):
                for site in sites_spec:
                    self.expanded_processes.append({"name": name, "sites": [int(site)], "strength": proc["strength"]})
                    self.index_list.append(i)
            else:
                for pair in sites_spec:
                    site_pair = [int(pair[0]), int(pair[1])]
                    self.expanded_processes.append({"name": name, "sites": site_pair, "strength": proc["strength"]})
                    self.index_list.append(i)
