#!/usr/bin/env python3
"""Finite-size scaling: memory entropy vs bath length ``L`` with shared probe sets.

Uses :class:`~mqt.yaqs.MemoryCharacterizer` for exact split-cut characterization.
One probe grid per cut ``c`` (RNG ``seed + c``) is reused for every ``(L, J)``.
...
def _entropy_at_cut(
    *,
    ...
    """Past-centered :math:`S_V` at one cut."""
    result = characterize(
        mc,
        ham,
        params,
    ...
    return float(metrics_from(result, int(cut))["entropy"])
...
def _initial_states(*, L: int, n_seeds: int, rng: np.random.Generator) -> list[np.ndarray]:
    ...
    for _ in range(n_seeds):
        psi_sys = random_qubit_state(rng).astype(np.complex128)
        psi = psi_sys
        for _ in range(L - 1):
            psi = np.kron(psi, z)
...
def run_benchmark(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    params = sim_params(dt=float(args.dt))
    ...
    for L in L_list:
        ...
        for J in J_list:
            ham = ising_chain(length=int(L), j=float(J), g=float(args.g))
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=Path, default=Path("results/finite_size"))
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--parallel", action="store_true", default=True)
    p.add_argument("--no-parallel", dest="parallel", action="store_false")
    p.add_argument("--k", type=int, default=K_DEFAULT, help="Instrument slots (cuts 1..k).")
