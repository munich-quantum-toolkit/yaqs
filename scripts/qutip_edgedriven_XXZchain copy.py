...
def qt_build_lindblad_operators(L, epsilon):
    ...
    c_ops.extend((
        np.sqrt(epsilon) * qt.tensor([sm] + [I] * (L - 1)),
        np.sqrt(epsilon) * qt.tensor([I] * (L - 1) + [sp]),
    ))
...
if __name__ == "__main__":
    ...
    for batch in range(N // batchsize):
        sim_params = PhysicsSimParams(
            measurements, T, dt, batchsize, max_bond_dim, threshold, order, sample_timesteps=sample_timesteps
        )
    ...
    for i, observable in enumerate(sim_params.observables):
        ...
        plt.axhline(
            y=steadystate_exact[i],
            linestyle="--",
            color="gray",
            label=f"⟨Z_{i}⟩ (exact steady state)" if i == 0 else None,
        )
