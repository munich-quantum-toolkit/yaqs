...
{func}`~mqt.yaqs.core.parallel.available_cpus` (re-exported as {func}`~mqt.yaqs.simulator.available_cpus`) is deliberately cgroup- and scheduler-aware. In priority order it honours:
...
```{code-cell} ipython3
from mqt.yaqs.core.parallel import get_parallel_context
