...
- **Long-range** gates always use the **generator MPO + two-site TDVP** path.

Set `gate_mode="hybrid_pauli"` for the former adaptive hybrid: long-range Pauli rotations
(`rxx`, `ryy`, `rzz`) **default to TDVP** and use exact Pauli-product enrichment only when
the projection-defect diagnostic exceeds `tdvp_projection_defect_tol` (including tangent-blind
cases where the projected generator is nearly zero).
