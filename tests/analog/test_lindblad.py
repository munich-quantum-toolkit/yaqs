...
def test_lindblad_amplitude_damping_L1() -> None:
    """Test single qubit amplitude damping with exact Lindblad solver."""
    L = 1
    initial_state = MPS(L, state="ones")  # |1>
    hamiltonian = MPO()
    hamiltonian.identity(L)
    for tensor in hamiltonian.tensors:
        tensor *= 0.0  # H = 0
...
def test_lindblad_unitary_rabi_L1() -> None:
    """Test single qubit unitary evolution (Rabi oscillation) with no noise."""
    L = 1
    initial_state = MPS(L, state="zeros")  # |0>

    hamiltonian = MPO.ising(L, J=0.0, g=-1.0)
    ...
    sim_params = AnalogSimParams(observables=[obs], elapsed_time=T, dt=dt, solver="Lindblad")
...
def test_lindblad_dephasing_L2() -> None:
    """Test 2-qubit system with local dephasing on one qubit."""
    L = 2
    initial_state = MPS(L, state="x+")  # |++> = (|0>+|1>)(|0>+|1>)/2
    ...
    sim_params = AnalogSimParams(observables=[obs0, obs1], elapsed_time=T, dt=dt, solver="Lindblad")
