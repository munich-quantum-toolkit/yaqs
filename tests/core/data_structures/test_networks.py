...
from mqt.yaqs.core.libraries.gate_library import GateLibrary, Id, X, Z, Destroy
...
def _embed_two_body(
    op1: np.ndarray, op2: np.ndarray, L: int, i: int
...
def _heisenberg_dense(
    L: int, Jx: float, Jy: float, Jz: float, h: float
) -> np.ndarray:  # noqa: N803
...
def test_from_pauli_sum_calls_create_term_for_each_term(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
