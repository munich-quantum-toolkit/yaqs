...
def test_exact_run_probe_diagnostics_hides_static_ctx_parameter() -> None:
...
def test_exact_run_probe_diagnostics_builds_static_ctx_internally(monkeypatch: pytest.MonkeyPatch) -> None:
    """ExactProbeProcess wires make_mcwf_static_context and simulate_sequences internally."""
    calls: dict[str, Any] = {}
...
def test_exact_diagnostics_use_cut_branch_weights(monkeypatch: pytest.MonkeyPatch) -> None:
    """simulate_probes_exact weights prod(step_probs[:cut])."""
    ...
    monkeypatch.setattr(exact_mod, "simulate_rollouts_traced", _fake_simulate)
    ...
    sim = AnalogSimParams(dt=0.1)
    psi0 = np.array([1.0 + 0.0j, 0.0 + 0.0j], dtype=np.complex128)
    _, weights, _ = simulate_probes_exact(
        probe_set=probe_set,
        operator=op,
        sim_params=sim,
        initial_psi=psi0,
...
def test_exact_run_probe_diagnostics_parallel_smoke() -> None:
