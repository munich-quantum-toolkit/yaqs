...
"""Tests for split-cut probes and run_probe_diagnostics."""
...
def test_run_probe_diagnostics_uses_object_backend() -> None:
    """run_probe_diagnostics delegates evaluation to a user-supplied process object."""
...
def test_comb_run_probe_diagnostics_returns_cut_weights() -> None:
    ...
    out = run_probe_diagnostics(process=comb, cut=1, k=1, n_pasts=4, n_futures=3, rng=rng)
...
def test_analytic_weights_match_exact_for_trivial_dynamics() -> None:
    ...
    _, w_exact, _ = simulate_probes_exact(
        probe_set=probe_set,
        operator=op,
        sim_params=_params(),
        initial_psi=_PSI0,
...
def test_dense_comb_vs_exact_probe_entropy() -> None:
    ...
    pauli_e, weights_e, _ = simulate_probes_exact(
        probe_set=probe_set,
    ...
    _m_e_raw, memory_matrix_e = assemble_weighted_matrix_from_probe(pauli_e, weights_e)
    out_exact = compute_spectrum(memory_matrix_e)
    out_comb = run_probe_diagnostics(process=comb, cut=2, k=2, probe_set=probe_set)
...
def test_mpo_comb_entropy_matches_dense() -> None:
    ...
    out_mpo = run_probe_diagnostics(process=mpo_comb, cut=1, k=1, probe_set=probe_set)
    out_dense = run_probe_diagnostics(process=dense, cut=1, k=1, probe_set=probe_set)
