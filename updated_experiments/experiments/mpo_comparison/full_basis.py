...
"""Local exhaustive probe catalog used by the Figure 4 reproduction.

This helper was copied from YAQS commit ``7238e1c8`` because the public
``operational_memory.full_basis`` module was later removed.  Keeping it local
preserves the probe ensemble used for the published comparison without
restoring an obsolete public API.
"""
...
from mqt.yaqs.characterization.memory.shared.interventions import encode_choi_features
    ...
    *,
    ...
    if past_full == 0:
        ...
        for alpha_tuple in product(*alpha_axes):
            alpha_label = (
                int(alpha_tuple[0]) if past_full == 1 else int(sum(a * n_maps**i for i, a in enumerate(alpha_tuple)))
            )
        ...
        for p_idx in range(n_states):
            for beta_tuple in product(*beta_axes):
                beta_label = (
                    int(beta_tuple[0])
                    if future_full == 1
                    else int(sum(b * n_maps**i for i, b in enumerate(beta_tuple)))
                )
...
def build_probe_set_from_catalog(
    ...
    for row, i in enumerate(past_idx):
        ...
        pairs_i: list[Any] = list(setting.past_pairs)
    ...
    for row, j in enumerate(future_idx):
        ...
        pairs_j = list(setting.future_pairs)
