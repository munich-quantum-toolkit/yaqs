...
def _panel_a_ylim(points: list[EntropyPoint]) -> tuple[float, float]:
    ...
    bulk = [value for p in points for value in (p.s_v, p.s_mpo) if p.j >= PANEL_A_J_MIN and value > PANEL_A_Y_FLOOR]
    if not bulk:
        return 1e-7, 1e-1
...
def _draw_j_legend_between(fig, ax_b, ax_c, layout: FigureLayout) -> None:
    ...
    j_handles = [Line2D([0], [0], color=J_COLORS[jv], lw=2.0, label=rf"$J={jv:g}$") for jv in SPECTRUM_JS]
    fig.canvas.draw()
...
def _draw_spectrum_panel(
    ...
    *,
    ...
    ax.xaxis.set_major_formatter(FuncFormatter(lambda x, _pos: f"{round(x)}"))
    if not show_xlabel:
