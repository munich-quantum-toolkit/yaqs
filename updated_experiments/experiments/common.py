...
def characterizer(*, parallel: bool = True, max_workers: int | None = None) -> MemoryCharacterizer:
    """Configured :class:`MemoryCharacterizer` for exact benchmarks."""
    return MemoryCharacterizer(parallel=parallel, max_workers=max_workers, show_progress=False)
...
def characterize(
    ...
    *,
    ...
    """Run raw IXYZ split-cut characterization via the public API."""
...
def characterize_custom_sequences(
    ...
    *,
    ...
    pauli_ixyz_ij, weights_ij = backend.evaluate_probes_weighted(
        probe_set,
        intervention_steps_list=psi_pairs_list,
    )
...
def evaluate_weighted_probes(
    ...
    *,
    ...
    return backend.evaluate_probes_weighted(probe_set, intervention_steps_list=psi_pairs_list)


def slice_probe_prefix(
    pauli_ixyz_ij: np.ndarray,
    ...
    if m < 1:
        ...
        raise ValueError(msg)
    p = np.asarray(pauli_ixyz_ij)
    if p.shape[0] < m or p.shape[1] < m:
        msg = f"cannot slice pauli_ixyz_ij shape={p.shape} to m={m}"
        raise ValueError(msg)
    p_sub = p[:m, :m, ...]
    ...
    if w.ndim >= 2 and w.shape[0] >= m and w.shape[1] >= m:
        ...
        msg = f"cannot slice weights_ij shape={w.shape} to m={m}"
        raise ValueError(msg)
    return np.asarray(p_sub), np.asarray(w_sub)


def entropy_from_responses(pauli_ixyz_sub: np.ndarray, weights_sub: np.ndarray) -> float:
    """Compute :math:`S_V` from sliced Pauli responses and branch weights."""
    response_matrix = assemble_response_matrix(pauli_ixyz_sub, weights_sub)
    ana = compute_spectrum(response_matrix)
