...
def run_simulator_with_intervention(
    ...
    rho_final = 0.5 * np.array([
        [1 + avg_z, avg_x - 1j * avg_y],
        [avg_x + 1j * avg_y, 1 - avg_z]
    ])
...
def test_held_out_prediction():
    ...
    print("\n" + "="*70)
    print("TEST 1: Held-Out Prediction (Generalization)")
    print("="*70)
    ...
    for i in range(num_tests):
        ...
        print(f"  Test {i+1}: θ=({theta0:.2f},{theta1:.2f}), φ=({phi0:.2f},{phi1:.2f}), Error={error:.6f}")
    ...
    if mean_error < 0.1 and max_error < 0.2:
        print("✓ PASS: PT correctly predicts held-out sequences")
        return True
    else:
        print("✗ FAIL: Prediction error too large")
        return False
