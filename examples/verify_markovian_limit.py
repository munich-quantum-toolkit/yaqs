...
from mqt.yaqs.tomography.tomography import run_process_tensor_tomography, _get_basis_states, _calculate_dual_frame

def test_algebraic_consistency():
    """Test 1: Frame self-consistency (floating point precision check)."""
    print("\n=== Test 1: Algebraic Frame Consistency (3 steps) ===")
    
    # 1. Setup random Hamiltonian for 2 qubits (System + Env)
    # Using random MPO to ensure general dynamics
    mpo = MPO()
    mpo.from_pauli_sum(
        terms=[(0.5, "X0 X1"), (0.3, "Y0 Z1"), (0.2, "Z0 Y1")],
        length=2,
    ...
    # This mode is faster.
    print("Running 3-step tomography (approx mode for speed)...")
    pt = run_process_tensor_tomography(
        operator=mpo,
        sim_params=sim_params,
        timesteps=timesteps,
        num_trajectories=10, 
        mode="pure_env_approx" 
    )
    ...
    print("\nChecking random frame sequences...")
    ...
    print(f"Max Algebraic Error: {max_error:.4e}")
    if max_error < 1e-12:
        print("✓ PASS: Algebraic consistency verified.")
        return True
    else:
        print("✗ FAIL: Algebraic consistency mismatch.")
        return False

def test_markovian_limit():
    """Test 2: Amplitude Damping on Environment (Markovian Limit)."""
    print("\n=== Test 2: Markovian Limit (Env Reset via Noise) ===")
    ...
    # 4. Run Tomography
    print("Running tomography with strong env damping...")
    ...
    print(f"\nDifference between sequences [0, +] and [1, +]: {diff:.4e}")
    ...
    if diff < 0.1:
        print("✓ PASS: Memory suppressed (Markovian limit verified).")
        return True
    else:
        print(f"✗ FAIL: Significant memory detected ({diff:.4e}). Damping insufficient?")
        return False
